...
<?
 ...
 $pet = $_REQUEST['pet'];
	    ...
	    $sql = " select emp.nombre_empresa , mov.id_movimiento, mov.id_expediente, mov.fecha_traslado, us.nombre, us.apellido, usua.nombre as d,usua.apellido as a, mov.comentarios from movimiento as mov inner join usuario as us
	...
	mov.id_expediente = expt.Id_expediente inner join empresa as emp on emp.id_empresa = expt.Id_empresa
		where  expt.Id_empresa = '$pet' order by mov.fecha_traslado asc";
$stmt = sqlsrv_query( $conn, $sql );
if( $stmt === false) {
    die( print_r( sqlsrv_errors(), true) );
