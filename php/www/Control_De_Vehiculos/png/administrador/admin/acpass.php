...
<?
 ...
 while($row) 
	...
	$sql = "select   nombre, apellido, usuario, id_usuario from dbo.usuario where id_usuario='$uen' and activo = 1";
$stmt = sqlsrv_query( $conn, $sql );
if( $stmt === false) {
    die( print_r( sqlsrv_errors(), true) );
}
...
while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
  ...
  <tr>
    ...
    <td><input type='password' name='npass' value='' required='required'></td>
  </tr>
  
  
  <tr>
    <td>Cambiar Contrase&ntilde;a? </td>
    <td> <select name='usde'> 
