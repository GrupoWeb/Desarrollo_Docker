...
<body aling="center">
<div id="container" class="ltr" align="center" style="width:80%" >
...
<form id="form5" name="for	m5" class="wufoo topLabel page" accept-charset="UTF-8" autocomplete="off" enctype="multipart/form-data" method="post" novalidate
	  ...
	  <?
	    ...
	    $sql = " select  count(*) as numero from expediente where id_ingreso=3 and id_solicitante= '$uen'";
$stmt = sqlsrv_query( $conn, $sql );
...
<h2>EXPEDIENTES ENVIADOS POLITICA INDUSTRIAL TOTAL: <? echo $no; ?> </h2>
</header>
...
width:620px; height:500px; z-index:1; overflow: scroll">-->
    <table align="center" border="1" bordercolor="#0066FF" width="300px">
      <tr>
        <th WIDTH="80px" >No. Expediente </th>
        <th WIDTH="80px" > Fecha</th>
        <th WIDTH="80px" >Hora </th>
        <th WIDTH="80px"> Solicitante </th>
        <th WIDTH="80px">Usuario que Recibe </th>
...
<?
	    ...
	    $sql = " select
  ex.Fecha_ingreso, us.nombre,us.apellido,  ex.comentario,ex.id_expediente, es.nombre as p , es.apellido as a
 ...
 inner join estado as est on ex.id_estado=est.Id_estado where id_ingreso=3 and ex.id_solicitante= '$uen' order by  ex.id_expediente desc";
$stmt = sqlsrv_query( $conn, $sql );
