...
<?
	    ...
	    $sql = " select  count(*) as numero from expediente where id_ingreso=2 and id_solicitante= '$uen'";
$stmt = sqlsrv_query( $conn, $sql );
...
<h2>EXPEDIENTES ENVIADOS DIACO TOTAL: <? echo $no; ?> </h2>
</header>
...
<?
 ...
 inner join estado as est on ex.id_estado=est.Id_estado where id_ingreso=2 and ex.id_solicitante= '$uen' order by  ex.id_expediente desc";
$stmt = sqlsrv_query( $conn, $sql );
