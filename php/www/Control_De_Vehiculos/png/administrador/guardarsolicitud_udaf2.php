...
<br></br>
 <table width='350' border='0' align='center'	>
   ...
   <tr>
	...
	$query =  "insert into documento (direccion,Descripcion,id_usuario,fechaad,nombrearchivo,tipoexpediente,id_documentoudaf) values ('$tmpfile','$a','$b',GETDATE(),'$nombre_archivo',2,2)";
   $row= sqlsrv_query($conn,$query ) or die (sqlsrv_errors()); 
	...
	if (strlen($userfile)>0)
	{
		...
		//agrega el correlatico SG-XX-2015
			...
			$query2 = "select max(id_expedienteudaf) as ar from dbo.expediente_udaf";
		  $stmt= sqlsrv_query($conn,$query2 ) or die (sqlsrv_errors()); 
	   ...
	   $sql ="insert into dbo.expediente_udaf (fecha_ingreso,comentario,id_usuario,id_dependencia,id_estado,id_documento,id_solicitud,numerodenit,id_solicitante,id_ingreso,id_correlativoftp,id_recibido ) 
	   values (GETDATE(), '$a','$b','$d','1',' $doc','$g','$ni','$uen','5','$idindex',20)";
		...
		$query3 = "select max(id_expedienteudaf) as aj from expediente_udaf";
