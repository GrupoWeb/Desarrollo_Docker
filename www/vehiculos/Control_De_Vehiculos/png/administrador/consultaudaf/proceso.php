...
<?
		 ...
		 $año = date ('Y');
	    $sql = "  select movq.id_movimiento, movq.id_expedienteudaf as idf, movq.fecha_traslado, us.nombre, us.apellido, usua.nombre as d, usua.apellido as a , movq.comentarios as coment, stp.Estado as nota, stp.paht as th  from movimientoudaf as movq inner join usuario as us
	on us.id_usuario = movq.id_usuario inner join usuario as usua on usua.id_usuario = movq.usuario_recibe inner join estado as stp on movq.id_recibido = stp.Id_estado
		where  movq.id_estado=2 and movq.usuario_recibe='$uen' order by movq.id_expedienteudaf desc
		";
$stmt = sqlsrv_query( $conn, $sql );
if( $stmt === false) {
    die( print_r( sqlsrv_errors(), true) );
...
while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) 
  ...
  $ex=($row["idf"]); 
        $t=$ex;     
	$thr=$row["th"];
echo "<tr><td> DF-".$row["idf"]."-".$año."</td> ";	   
echo "<td >",date_format ($date,'d-m-y'),"</td> ";
echo "<td>",date_format ($date, 'H:i:s'),"</td> ";
echo "<td>".$row["p"]."".$row["a"]."</td> ";
echo "<td>".$row["nombre"]." ".$row["apellido"]." </td> ";
echo "<td>".$row["coment"]."</td>";
echo "<td> <a href='../versolicitududaf/versolicitud2.php?ex=$t' >  <img src='images/lupas.png' width='24' height='22'> </td>";
echo "<td><a href='recibido2.php?ex=$t' > <img src='images/$thr' width='24' height='22'> </td>";
