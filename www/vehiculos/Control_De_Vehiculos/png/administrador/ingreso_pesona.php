...
<form action="guardar_persona.php" method="post" enctype="multipart/form-data" name="form1">
  
<header id="header" class="info">
<h2>Ingreso de Expedientes Corredores de Bienes </h2>

<p>
  <? include("consulta/fecha.php");  ?>
</p>
<p><img src="../logos_expe/mineco.png" width="193" height="90"></p>
</header>
...
<div>
  ...
  <p>    <strong>Nombre de </strong><span class="desc"><strong>Persona:</strong>
...
?>
  ...
  <label class="desc" id="title4" for="Field4"></label><p>&nbsp;</p>
  </div >
...
<div>
  <select name="solic" required="required">
    <?
	    ...
	    $sql = "SELECT nombre_envio, id_tipoenvio FROM dbo.tipo_envio where activo = 1 order by nombre_envio asc";
$stmt = sqlsrv_query( $conn, $sql );
if( $stmt=== false) {
    die( print_r( sqlsrv_errors(), true) );
}	
?>
<option value="<? $fila['id_tipoenvio'] ?>">-Seleccione-</option> 
<? 
while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
print "<option value=\"".$row["id_tipoenvio"]."\">".$row["nombre_envio"]."</option>"; 
}
sqlsrv_free_stmt( $stmt);		
?>
  </select>
