...
<form id="form5" name="for	m5" class="wufoo topLabel page" accept-charset="UTF-8" autocomplete="off" enctype="multipart/form-data" method="post" novalidate
	  ...
	  <?
	    ...
	    $sql = " select  count(*) as numero from expediente where id_ingreso=3 and id_usuario= '$uen'";
$stmt = sqlsrv_query( $conn, $sql );
...
<h2>EXPEDIENTES RECIBIDOS POLITICA INDUSTRIAL TOTAL: <? echo $no; ?> </h2>
</header>
...
<?
 ...
 inner join estado as est on ex.id_estado=est.Id_estado where id_ingreso=3 and ex.id_usuario= '$uen' order by  ex.id_expediente desc";
$stmt = sqlsrv_query( $conn, $sql );
