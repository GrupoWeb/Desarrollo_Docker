...
<form id="form5" name="for	m5" class="wufoo topLabel page" accept-charset="UTF-8" autocomplete="off" enctype="multipart/form-data" method="post" novalidate
	  ...
	  <?
	    ...
	    $sql = " select  count(*) as numero from expediente where id_ingreso=4 and id_solicitante= '$uen'";
$stmt = sqlsrv_query( $conn, $sql );
...
<h2>EXPEDIENTES ENVIADOS SECRETARIA TOTAL: <? echo $no; ?> </h2>
</header>
...
<?
 ...
 inner join estado as est on ex.id_estado=est.Id_estado where id_ingreso=4 and ex.id_solicitante= '$uen' order by  ex.id_expediente desc";
$stmt = sqlsrv_query( $conn, $sql );
