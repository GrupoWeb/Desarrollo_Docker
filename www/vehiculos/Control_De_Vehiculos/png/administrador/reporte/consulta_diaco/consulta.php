...
<?
	    ...
	    $sql = " select  count(*) as numero from expediente where id_ingreso=2 and id_usuario= '$uen'";
$stmt = sqlsrv_query( $conn, $sql );
...
<h2>EXPEDIENTES RECIBIDOS DIACO TOTAL: <? echo $no; ?> </h2>
</header>
...
<?
 ...
 inner join estado as est on ex.id_estado=est.Id_estado where id_ingreso=2 and ex.id_usuario= '$uen' order by  ex.id_expediente desc";
$stmt = sqlsrv_query( $conn, $sql );
